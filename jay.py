...
def opening_url(curl,url):
    ...
    try:
        ...
        print('Page ',url,' retrieve failure')
        return 0
    ...
    return soup,url
def jsparser_links(curl,url,df):
    sop=opening_url(curl,url)
    if(sop==0):
        return 0,0 
    soup=sop[0]
...
def read_js_list():
    ...
    data=(dataa.split(' '))
    return data
def scraper(curl,url,dfs):
    sop=opening_url(curl,url)                                            #Page retrieval request through the function calling
    if(sop==0):
        return 0
    if(type(sop)==tuple):
        soup=sop[0]
    ...
    if(len(js_src)):
        for jrr in js_src:                                      #Looping through the javascript files serially
            if(url.count(jrr)<=1):
                ur=url+str(jrr)
                df=jsparser_links(curl,ur,dfs[1])                               #Function called for the related javascript files parsing
                if(df[1]):
                    dfs[1]=df[0]
    dfs=[dfs[0],dfs[1]]
    return dfs
    ...
    if(args.url is not None):          #Checking if the user provided with the url
        ...
        for url in urls:
            ...
            if(dfs!=0):
                ...
                print('******** ',slno,'th website not scraped*******')
            slno=slno+1
    return 0        
